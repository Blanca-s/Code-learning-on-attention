...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1): # d_model is the length of the embedding, which is typically 512.
        ...
        self.h = h
        self.linears = clones(nn.Linear(d_model, d_model), 4) # four stacked fully connected layers whose input and output are all d_model dimensions.
        self.attn = None
        self.dropout = nn.Dropout(p=dropout) # p is the probability of an element to be zeroed. And we know that dropout has been assigned to 0.1.

    def forward(self, query, key, value, mask=None): # forward function will be automatically called when we call the object of this class.
        if mask is not None:
            # Same mask applied to all h heads.
            mask = mask.unsqueeze(1) # Add a dimension at the 1th dimension of the mask tensor.
        nbatches = query.size(0) # Tensor "query" has four dimensions: batch size, number of heads, number of words, length of the embedding.
...
def clones(module, N):
    return nn.ModuleList([copy.deepcopy(module) for _ in range(N)]) # nn.ModuleList is a special list that could store a list of neural network modules.
# copy.deepcopy(module) will create independent copies of the module, which is different from the normal copy. Thus, those copies will not share weights and other parameters.
